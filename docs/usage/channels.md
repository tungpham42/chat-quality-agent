...
CQATP hỗ trợ 2 kênh chat: **Zalo OA** và **Facebook Messenger**. Bạn có thể kết nối nhiều kênh cùng lúc.
...
- **CQATP phải có tên miền + SSL** (Zalo yêu cầu callback URL là HTTPS). Xem [Tên miền & SSL](/guide/domain-ssl)

### Bước 1: Bật SSL cho CQATP

Zalo yêu cầu callback URL phải là HTTPS. Nếu chưa bật SSL, hãy cấu hình trước:

- Trỏ domain về IP VPS (ví dụ `cqatp.yourdomain.com`)
...
2. Nhập domain CQATP (ví dụ `cqatp.yourdomain.com`)
...
::: tip Dùng TXT Record
Nên dùng phương thức **DNS TXT Record** vì không cần thay đổi gì trên server CQATP. Các phương thức khác (upload file HTML, thêm meta tag) yêu cầu tùy chỉnh thêm.
:::
...
::: danger Quan trọng
Quyền **"Quản lý tin nhắn người dùng"** là **bắt buộc** để CQATP đọc được tin nhắn.
:::

3. Tại mục **"Official Account Callback Url"**, nhập URL callback: `https://cqatp.yourdomain.com/api/v1/channels/zalo/callback`
4. Click **"Cập nhật"**.
...
### Bước 7: Kết nối kênh trên CQATP

1. Trong CQATP, vào **Kênh chat** → click **"+ Kết nối kênh mới"**
...
![Form kết nối trên CQATP](/screenshots/zalooa/zalooa-12.png)
...
5. Zalo redirect về CQATP. Hệ thống tự động nhận authorization code và lấy OA Access Token.
...
- **Token hết hạn**: Access token Zalo hết hạn sau 90 ngày. CQATP tự động refresh, nhưng nếu token bị revoke, bạn cần **xác thực lại**.
