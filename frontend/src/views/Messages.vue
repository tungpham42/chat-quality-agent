<template>
  <div>
    <h1 class="text-h5 font-weight-bold mb-4">
      {{ $t("nav_messages") }}
      <span
        v-if="conversationStore.total"
        class="text-body-2 text-grey font-weight-regular ml-2"
        >({{ conversationStore.total }})</span
      >
    </h1>
    ...
    <v-row>
      ...
      <v-col
        cols="12"
        :md="selectedConvId ? 5 : 12"
        :lg="selectedConvId ? 4 : 12"
      >
        <!-- Filters -->
          ...
          <v-card>
            ...
            <v-card-text>
              <div class="text-body-2 text-grey mb-3">
                Xuất toàn bộ cuộc chat trong khoảng thời gian để AI đọc và phân
                tích.
              </div>
              <v-row dense>
                <v-col cols="6">
              ...
              <v-select
                ...
                :items="[
                  { title: 'Text (cho AI đọc)', value: 'txt' },
                  { title: 'CSV (bảng tính)', value: 'csv' },
                ]"
                label="Định dạng"
                density="compact"
                variant="outlined"
        ...
        <v-card variant="outlined">
          ...
          >
            ...
            >
              ...
              >
                <template #prepend>
                  <v-avatar
                    :color="conv.channel_type === 'facebook' ? 'blue' : 'green'"
                    size="32"
                    class="mr-3"
                  >
                ...
                <v-list-item-subtitle class="text-caption">
                  ...
                  <v-chip
                    v-if="evaluationMap[conv.id]"
                    size="x-small"
                    :color="
                      evaluationMap[conv.id] === 'PASS' ? 'success' : 'error'
                    "
                    variant="tonal"
                    class="mr-1"
                  >
          ...
          <v-card-text v-else-if="!loading" class="text-center py-8">
            ...
            <div class="text-grey-darken-1 mt-2">
              Tin nhắn sẽ hiện ở đây sau khi kết nối và đồng bộ kênh chat.
            </div>
            <v-btn
              variant="text"
              color="primary"
              size="small"
              class="mt-2"
              :to="`/${tenantId}/channels`"
              >Đi tới kênh chat</v-btn
            >
          </v-card-text>
      ...
      <v-col v-if="selectedConvId" cols="12" md="7" lg="8">
        ...
        >
          ...
          <v-card-title class="d-flex align-center pa-4">
            ...
            />
            <v-avatar
              :color="selectedConvChannelType === 'facebook' ? 'blue' : 'green'"
              size="36"
              class="mr-3"
            >
              <v-icon color="white" size="18">
                {{
                  selectedConvChannelType === "facebook"
                    ? "mdi-facebook-messenger"
                    : "mdi-chat"
                }}
              </v-icon>
            ...
            <div class="flex-grow-1">
              <div class="text-subtitle-1 font-weight-medium">
                {{
                  conversationStore.currentConversation?.customer_name ||
                  $t("msg_unknown_customer")
                }}
              </div>
              <div class="text-caption text-grey">
                {{ conversationStore.currentConversation?.message_count }}
                {{ $t("msg_messages_count") }}
              </div>
            </div>
            <v-btn
          ...
          >
            ...
            <v-tab value="qc">
              ...
              Đánh giá
              <v-chip
                v-if="qcGroups.length"
                size="x-small"
                color="grey"
                variant="tonal"
                class="ml-1"
                >{{ qcGroups.length }}</v-chip
              >
            </v-tab>
              ...
              >
                ...
                >
                  ...
                  <div v-if="hasAttachments(msg)" class="mt-1">
                    <template
                      v-for="(att, i) in parseAttachments(msg)"
                      :key="i"
                    >
                      <div v-if="isImageAttachment(att)" class="mb-1">
                        <img
                          v-if="
                            authImageCache[getAttachmentUrl(att)] &&
                            authImageCache[getAttachmentUrl(att)] !== 'loading'
                          "
                          ...
                          "
                          @click="
                            lightboxSrc = authImageCache[getAttachmentUrl(att)]
                          "
                        ...
                        />
                        <v-chip
                          v-else-if="!getAttachmentUrl(att)"
                          size="x-small"
                          variant="tonal"
                          color="grey"
                        >
                  ...
                  <div
                    ...
                    :class="
                      msg.sender_type === 'agent'
                        ...
                        : 'text-grey'
                    "
                    style="opacity: 0.6; font-size: 10px"
                  >
          ...
          >
            ...
            <div v-else-if="qcGroups.length === 0" class="text-center py-8">
              ...
              <div class="text-grey mt-3">
                Cuộc chat này chưa được đánh giá chất lượng.
              </div>
            </div>
            <div v-else>
              <v-card
              ...
              >
                <v-card-text class="pa-3">
                  <div class="d-flex align-center mb-2">
                    ...
                    <v-chip
                      v-if="getQcScore(g) != null"
                      size="x-small"
                      variant="tonal"
                      class="mr-2"
                      >{{ getQcScore(g) }}/100</v-chip
                    >
                    <span class="text-body-2 font-weight-medium flex-grow-1">{{
                      g.job_name
                    }}</span>
                    <span class="text-caption text-grey">{{
                      formatTime(g.evaluated_at)
                    }}</span>
                  </div>
                  <div
                    v-if="getQcReview(g)"
                  ...
                  <v-btn
                    v-if="getQcViolations(g).length > 0"
                    size="x-small"
                    variant="text"
                    color="primary"
                    @click="toggleQcExpand(g.job_run_id)"
                  >
                    {{
                      expandedQc[g.job_run_id]
                        ? "Thu gọn"
                        : `Xem chi tiết (${getQcViolations(g).length} vấn đề)`
                    }}
                  </v-btn>
                  <div v-if="expandedQc[g.job_run_id]" class="mt-2">
                    <div
                      v-for="(v, idx) in getQcViolations(g)"
                      :key="idx"
                      class="mb-2"
                    >
                      <div class="d-flex align-center mb-1">
                        <v-chip
                          size="x-small"
                          :color="
                            v.severity === 'NGHIEM_TRONG' ? 'error' : 'warning'
                          "
                          variant="tonal"
                          class="mr-2"
                        >
                          {{
                            v.severity === "NGHIEM_TRONG"
                              ? "Nghiêm trọng"
                              : "Cần cải thiện"
                          }}
                        </v-chip>
                      ...
                      <div
                        class="text-body-2 bg-orange-lighten-5 pa-2 rounded"
                        style="font-size: 13px; border-left: 3px solid #ff9800"
                      >
                        {{ v.evidence }}
                      </div>
                    </div>
          ...
          <!-- Classification tab -->
          <div
            v-show="detailTab === 'classification'"
            class="flex-grow-1 overflow-y-auto pa-4"
          >
            <div v-if="loadingEvaluation" class="text-center py-8">
              <v-progress-circular indeterminate />
            ...
            <div v-else-if="classGroups.length === 0" class="text-center py-8">
              ...
              <div class="text-grey mt-3">
                Cuộc chat này chưa được phân loại.
              </div>
            </div>
            <div v-else>
              <v-card
              ...
              >
                <v-card-text class="pa-3">
                  ...
                  <div class="d-flex flex-wrap ga-1 mb-2">
                    <v-chip
                      v-for="tag in getClassTags(g)"
                      :key="tag"
                      size="small"
                      :color="msgTagColor(tag)"
                      variant="tonal"
                    >
...
async function loadEvaluationMap() {
  try {
    const { data } = await api.get(
      `/tenants/${tenantId.value}/conversations/evaluated`,
    );
...
async function doExport() {
  ...
  try {
    ...
    if (exportChannelType.value)
      url += `&channel_type=${exportChannelType.value}`;
    const { data } = await api.get(url, {
      responseType: exportFormat.value === "csv" ? "text" : "text",
    });
    if (data?.error) {
...
const qcGroups = computed(() => {
  ...
  return evaluation.value.groups.filter(
    (g: any) => g.job_type === "qc_analysis",
  );
});
const classGroups = computed(() => {
  if (!evaluation.value?.groups) return [];
  return evaluation.value.groups.filter(
    (g: any) => g.job_type === "classification",
  );
});

const MSG_TAG_COLORS = [
  "#7E57C2",
  "#1E88E5",
  "#00897B",
  "#FB8C00",
...
function msgTagColor(tag: string): string {
  ...
  return idx >= 0
    ? MSG_TAG_COLORS[idx % MSG_TAG_COLORS.length]
    : MSG_TAG_COLORS[0];
}

// QC group helpers
function getQcVerdict(g: any): string {
  const ev = g.results?.find(
    (r: any) => r.result_type === "conversation_evaluation",
  );
  return ev?.severity || "";
}
function getQcScore(g: any): number | null {
  ...
  try {
    return JSON.parse(ev.detail)?.score ?? null;
  } catch {
    return null;
  }
}
...
function getClassSummary(g: any): string {
  ...
  if (!ev?.detail) return "";
  try {
    return JSON.parse(ev.detail)?.summary ?? "";
  } catch {
    return ev?.evidence || "";
  }
}
...
const channelOptions = computed(() => {
  ...
  if (filterChannelType.value) {
    filtered = filtered.filter(
      (c) => c.channel_type === filterChannelType.value,
    );
  }
...
async function selectConversation(convId: string, tab?: string) {
  ...
  try {
    const { data } = await api.get(
      `/tenants/${tenantId.value}/conversations/${convId}/evaluations`,
    );
...
function onImageError(event: Event, att: any) {
  ...
  fallback.className =
    "v-chip v-chip--size-x-small v-theme--light v-chip--density-default v-chip--variant-tonal";
  fallback.style.cssText =
    "font-size: 10px; padding: 0 8px; height: 24px; display: inline-flex; align-items: center; border-radius: 12px; background: rgba(0,0,0,0.06);";
...
onMounted(async () => {
  ...
  if (route.query.conv) {
    ...
    try {
      ...
      const { data } = await api.get(
        `/tenants/${tenantId.value}/conversations/${convId}/page`,
